...
class TextData:
    ...
    def create_batch(self, samples):
        ...
        for i in range(batch_size):
            ...
            batch.encoderSeqs.append([self.sr_word2id[item] if item in self.sr_word2id.keys() else 3  for item in list((sample[0])) ])  # Reverse inputs (and not outputs), little trick as defined on the original seq2seq paper
            # batch.encoderSeqs.append([self.sr_word2id[item] if item in self.sr_word2id.keys() else 3  for item in list(reversed(sample[0])) ])  # Reverse inputs (and not outputs), little trick as defined on the original seq2seq paper
            # print('sample 1', sample[1])
            batch.decoderSeqs.append([self.sr_word2id[self.goToken]] + [self.sr_word2id[item] if item in self.sr_word2id.keys() else 3 for item in list(sample[1])] + [self.sr_word2id[self.eosToken]])  # Add the <go> and <eos> tokens
