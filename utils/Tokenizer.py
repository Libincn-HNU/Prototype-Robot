...
class Tokenizer(object):
    ...
    def __init__(self, corpus_files, task_type, emb_type):
        ...
        self.word2idx['<BEG>'] = 2
        self.word2idx['<END>'] = 3
        self.idx2word[2] = self.word2idx['<BEG>']
        self.idx2word[3] = self.word2idx['<END>']
    ...
    def __set_embedding_matrix(self, word2idx):
        ...
        if self.emb_type == 'random':
            embedding_matrix = np.zeros((len(word2idx) + 4, 300))
        elif self.emb_type == 'tencent':
            embedding_matrix = np.zeros((len(word2idx) + 4, 200))
            ...
            embedding_matrix[self.word2idx['<BEG>']] = np.zeros(200)  # Padding
            embedding_matrix[self.word2idx['<END>']] = np.zeros(200)  # Padding
